...
def translate_text(text, target_lang, provider="DeepL"):
    ...
    try:
        if provider == "DeepL":
            ...
            detected_lang = source_lang_result.detected_source_lang_normalized
...
def main():
    ...
    if provider == "DeepL":
        try:
            ...
            # Find the Chinese language code
            chinese_code = next((code for code, name in target_languages.items() 
                               if "Chinese" in name or code == "ZH"), 'BG')
            st.write("Chinese code found:", chinese_code)
        except Exception as e:
    ...
    with col_lang:
        ...
        # Set default to the found Chinese code or first language
        default_index = target_langs.index(chinese_code) if chinese_code in target_langs else 0
